...
double login(HashMap* map, char* username) {
    ...
    if (choice == 's' || choice == 'S') {
        ...
        if (user && strcmp(user->password, password) == 0) {
            ...
            if (retry == 'i' || retry == 'I') {
                return login(map, username); // Llamar recursivamente a login para intentar nuevamente
...
double watch(horse horses[H_NUM]) {
    ...
    while (strlen(winner) == 0) {
        ...
        for (i = 1; i < H_NUM; i++) {
            ...
            printf("%-*s|", NAME_WIDTH, horses[i].name);  // Nombre fuera de la pista, alineado a la izquierda
